...
screen input(prompt):
    ...
    frame:
        ...
        vbox:
            ...
            # ★ 無理やり補助テキスト
            text "Enterキーで確定":
                font "fonts/NotoSansJP-Regular.ttf" # フォント指定
                size 28
                color "#888"
                xalign 0.5
...
screen main_menu():
    ...
    #   ※仮で同じ画像でもOK
    # ------------------------------------------------------------
    if all(e in clears for e in ["A", "B"]):
        add "images/bg/bg_last_scene_04.png" xysize (config.screen_width, config.screen_height)
    else:
        add "images/bg/bg_pension_front_02.png" xysize (config.screen_width, config.screen_height)
    ...
    add Solid("#E5E5E599"):
        xalign 0.5
        yalign 0.35
        xsize 1000   # ← 幅を制限（ここ重要）
        ysize int(config.screen_height * 0.35)

    # --------------------------------------
    # タイトルロゴ
    # --------------------------------------
    add "images/logo_yamaoku_pension_no_satsui.png":
        xalign 0.5         # 横中央
    ...
    if gui.show_name: # ここが「そもそも出すかどうか」のスイッチ
        ...
        vbox:
            ...
            text "[config.version]":                  # バージョン表記（その下に出てるやつ）
                color "#DDDDDD"                     # インデントで色変更ができた。これで正しいのかわからない。
...
screen hint_screen():
    ...
    frame:
        ...
        vbox:
            ...
            text "ヒント" size 38
            text """            
            この物語には、**大きく2つの結末**があります。
            Aエンド：基本的に、このエンドに到達します。
            Bエンド：以下の手順で到達します。
            二階廊下で吹き抜けのロビーを眺めた後に出現する選択肢で、
            ２　「まず荷物置こうよ。」僕は軽く制止する。
            を選び、部屋に向かいます。
            その後、出現する選択肢で
            ２　「ベッドどっちにする？僕、窓際がいい！」一緒になってはしゃいだ。
            を選び、窓の外を見ます。【フラグ１】
            その後、ロビーで撮影の計画を練るシーンでの選択肢で
            ２　ここは木下さんに相談だ
            を選びます。【フラグ２】        
            """ size 28
        ...
        vbox:
            ...
            viewport:
                ...
                vbox:
                    ...
                    ゲーム制作の過程でChatGPTに相談し、
                    背景から人物を切り抜くためのツール「ふわっとトリム」を制作しました。
                    ノベルゲームを作っていく過程で「ノベルゲームといえば影絵」というイメージから、
                    「透明部分を活かして影絵を作れないか」と考え、
                    ChatGPTに相談して影絵作成ツール「透け影工房」を制作しました。
                    ...
                    ただ、小説を書いたことがないような人間でも、ある程度まとまった文章が書けるようになったのは、
                    やはりLLMの進歩を感じます。
                    ...
                    今作のスクリプトのベースとなっているのは、以前 AI を活用して Ren'Py で制作したノベルゲーム
                    『魔人の棲む塔 - Ren’Py Remake』
                    のスクリプトです。
...
# ・persistent.endings_cleared に "A","B" が入る想定
# ------------------------------------------------------------
